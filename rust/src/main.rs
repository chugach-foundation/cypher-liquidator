...
async fn main() -> Result<(), LiquidatorError> {
    ...
    info!("Initializing rpc client for cluster-{} with url: {}", liquidator_config.cluster, cluster_config.rpc_url);
    ...
    match tokio::join!(liq_t) {
        (Ok(_),) => (),
        (Err(e),) => {
            warn!("An error occurred while joining with the liquidator task: {}", e.to_string());
        }
    }
